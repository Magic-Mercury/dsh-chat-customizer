...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /**
     * Per-session-row agent label hole (declared by the WorkspaceBrowser
     * entry): a registrant renders a small tag beside the session title
     * naming the agent preset the session runs. Absent an occupant the row
     * renders without the tag.
     */
    'sidebar.workspaces.row.agent': { kind: 'single'; scope: 'root'; owner: SessionRowAgentOwnerProps }
  }
}

/**
 * Owner share of the session-row agent label hole: the session facts a tag
 * needs. The occupant (e.g. a contacts plugin with the preset roster) maps
 * the preset id to a display name.
 */
export interface SessionRowAgentOwnerProps {
  /** The session whose row is rendering. */
  sessionId: SessionId
  /** The agent preset id the session runs, absent without presets. */
  agentPreset?: string
}
...
export type WorkspaceBrowserProps =
  ...
  & PropsRenderSlots<'sidebar.workspaces.directoryFlow' | 'sidebar.workspaces.row.agent'>
