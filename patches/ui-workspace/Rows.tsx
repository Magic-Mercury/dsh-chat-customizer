 ...
 */
export function SessionNodeItem({ node, currentId, now, onOpen, onRename, onFork, onArchive, drag, flat = false, renderAgent, t }: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  /** Optional per-row agent label renderer (the browser's row-agent slot). */
  renderAgent?: ((owner: { sessionId: SessionNode['id']; agentPreset?: string }) => ReactNode) | undefined
  t: RowTranslate
}) {
  const row = node
  ...
  const ownRow = (
    ...
    >
      ...
      {/* The agent tag rides between the title and the timestamp; a blank
          placeholder row has no agent to name. */}
      {node.agentPreset !== undefined && renderAgent?.({ sessionId: node.id, agentPreset: node.agentPreset })}
